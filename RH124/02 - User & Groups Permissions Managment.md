...
----------------------------------------------------------------------------------------------------------------
**id <user>**---> Show all details of a user (UID, GID, secondry groups)
...
**usermod -c "ubuntu" -s /sbin/nologin** ---> Lock user from logging in and acuiring a a bash.
...
**chsh <user> /bin/bash** ---> change the login shell of a user 
															 (like /sbin/nologin or /bin/bash).

**su - <user>** ---> Raise your previleges to a user you know his password.
...
*we can also use grant groups to have a certain user previliges:*
**%admins 			ALL=(root) 		NOPASSWD:/usr/bin/dnf**

*We can also have drop-in files that mimic adding entries to sudoers, but being separetely named:*
...
**wx** ---> Create/delete files inside - REGRDLESS of being an owning user/group or not.
For files:
**r** ---> Read.
**w** ---> Edit.
**rx** --> Execute.
...
1)
**suid** -->  File temporarly runs as **its owner**, not as the user who launched it.
...
*Effect vanishes once session is terminated, **add your unmask to ~/.bashrc** to make the effect permanent.*
